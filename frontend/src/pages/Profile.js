...
const Profile = () => {
  ...
  const getprfiledata = async () => {
    const profileData = await axios.get(
      "http://localhost:8000/student/" + userId
    );
    ...
    <>
      <Container style={{ marginTop: "30px" }}>
        <Snackbar
          ...
          onClose={imageUploadstatus}
        >
          <Alert onClose={imageUploadstatus} severity="success">
            Image Updated Successfully
          </Alert>
        </Snackbar>
        <Snackbar
          open={updateProfileCheck}
          autoHideDuration={5000}
          onClose={profileUpdate}
        >
          <Alert onClose={profileUpdate} severity="success">
        ...
        <Grid container border={1}>
          <Grid item xs={12} sm={8}>
            <Grid container>
              <Grid item xs={12} sm={4}>
                <img
                  src={"http://localhost:8000/" + data.profile}
                  alt=""
                  height="270"
                  width="100%"
                />
          ...
          <Grid item xs={12} sm={4}>
            <Typography variant="h6">OTHER STUDENTS :=</Typography>
            <div>
              <List>
                {userList
                  .filter((filterData) => filterData.id !== userId)
                  .map((name, index) => (
                    <ListItem key={index}>
                      <ListItemAvatar>
                        <Avatar>
                          <img
                            src={"http://localhost:8000/" + name.profile}
                            alt=""
                          />
                        </Avatar>
                      </ListItemAvatar>
